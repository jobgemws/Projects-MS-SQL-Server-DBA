...
set @str=null;
set @iscreated=0;
if(not exists(select top(1) 1 from sys.views where [name]=N'vDelIndexInclude' and [schema_id]=SCHEMA_ID(N'srv')))
begin
	set @iscreated=1;

	set @str='CREATE view [srv].[vDelIndexInclude] as
	/*
	  Погорелов А.А.
...
begin
	set @str='ALTER view [srv].[vDelIndexInclude] as
	/*
	  Погорелов А.А.
	  Поиск перекрывающихся(лишних) индексов.
	  Если поля индекса перекрываются более широким индексом в том же порядке следования полей начиная с первого поля, то 
...
set @str=null;
set @iscreated=0;
if(not exists(select top(1) 1 from sys.views where [name]=N'vReadWriteTables' and [schema_id]=SCHEMA_ID(N'inf')))
begin
	set @iscreated=1;

	set @str='create view [inf].[vReadWriteTables] as
...
begin
	set @str='ALTER view [inf].[vReadWriteTables] as
	-- Чтение/запись таблицы
	-- Кучи не рассматриваются, у них нет индексов
	-- Только те таблицы, к которым обращались после запуска SQL Server
